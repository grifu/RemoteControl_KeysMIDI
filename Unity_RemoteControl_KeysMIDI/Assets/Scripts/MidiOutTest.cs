...
sealed class MidiOutTest : MonoBehaviour
{
    ...
    {
        int nota = 0;
        teclas = GameObject.Find("TeclasInput");
        nota = teclas.GetComponent<LogRawKeyInput>().notaMIDI;
        Debug.Log("nota MIDI = "+nota);
        Destroy(this, 1);
        ...
        foreach (var port in _ports) port?.SendNoteOn(0, nota, 100);
        yield return new WaitForSeconds(0.1f);
        foreach (var port in _ports) port?.SendNoteOff(0, nota);
        yield return new WaitForSeconds(0.1f);
